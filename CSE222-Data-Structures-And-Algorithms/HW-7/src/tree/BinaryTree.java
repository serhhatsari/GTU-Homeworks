...
public class BinaryTree<E> implements Serializable {
  ...
  public void inOrderTraverseWithSortedArray(E[] arr) {
    array = arr;
    index = 0;
    inOrderTraverseWithSortedArray(root);
  }
   ...
   * @param sb The string bufer to save the output
   */
  private void inOrderTraverseWithSortedArray(Node<E> node) {
    if (node == null) {
      //do nothing
    } else {
      inOrderTraverseWithSortedArray(node.left);
      node.data = array[index];
      index++;
      inOrderTraverseWithSortedArray(node.right);
    }
  }
