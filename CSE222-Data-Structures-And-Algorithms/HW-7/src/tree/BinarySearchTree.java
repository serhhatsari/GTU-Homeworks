...
public class BinarySearchTree<E extends Comparable<E>>
  ...
  public void postOrderTraverseForConverting() {
    postOrderTraverseForConverting(root);
  }
   ...
   */
  private void postOrderTraverseForConverting(Node<E> node) {
    if (node == null) {} else {
      postOrderTraverseForConverting(node.left);
      postOrderTraverseForConverting(node.right);
      if (findBalanceHeight(node) == -2 && findBalanceHeight(node.left) == -1) {
        rotateRight(node);
      } else if (
        findBalanceHeight(node) == -2 && findBalanceHeight(node.left) == 1
      ) {
        rotateLeft(node.left);
        rotateRight(root);
      } else if (
        findBalanceHeight(node) == 2 && findBalanceHeight(node.right) == +1
      ) {
        rotateLeft(node);
      } else if (
        findBalanceHeight(node) == 2 && findBalanceHeight(node.right) == -1
  ...
  private int findBalanceHeight(Node<E> node) {
    int leftMaxDepth = calculateMaxDepth(node.left);
    int rightMaxDepth = calculateMaxDepth(node.right);
    return leftMaxDepth - rightMaxDepth;
  }

  private int calculateMaxDepth(Node<E> node) {
    if (node == null) return -1; else {
      /* compute the depth of each subtree */
      int leftNode = calculateMaxDepth(node.left);
      int rightNode = calculateMaxDepth(node.right);

      /* use the larger one */
      if (leftNode > rightNode) return (leftNode + 1); else return (
        rightNode + 1
      );
    }
  }
