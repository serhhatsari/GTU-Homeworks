...
public class BinarySearchTree<E extends Comparable<E>>
  ...
  /**
   * It returns an ArrayList of all the nodes in the tree.
   *
   * @return An ArrayList of nodes.
   */
  protected ArrayList<E> getNodes() {
  ...
  /**
   * If the local root is not null, add the local root's data to the list, then recursively call getNodes
   * on the local root's left and right children.
   *
   * @param localRoot The current node we're looking at.
   * @param nodes The ArrayList that will hold the nodes of the tree.
   */
  private void getNodes(Node<E> localRoot, ArrayList<E> nodes) {
    if (localRoot == null) {
      return;
    }
    nodes.add(localRoot.data);
    getNodes(localRoot.left, nodes);
    getNodes(localRoot.right, nodes);
  }
}
